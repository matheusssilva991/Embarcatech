                                            ...
                                            const int32_t lhs_cols_offset)
{
    ...
    for (; i_items <= (lhs_rows - 4); i_items += 4)
    {
        for (int i = 0; i < rhs_rows; i++)
        {
            ...
            __ASM volatile(" .p2align 2                             \n"
                           ...
                           [out1] "=Te"(acc_n1),
                           [out2] "=Te"(acc_n2),
                           [out3] "=Te"(acc_n3) : [cnt] "r"(rhs_cols) : "q0", "q1", "q2", "q3", "q4", "memory", "r14");
#endif
    ...
    for (; i_items < lhs_rows; i_items++)
    {
        ...
        for (int i = 0; i < rhs_rows; i++)
        {
            ...
            __ASM volatile(" .p2align 2                             \n"
                           ...
                           "   letp            lr, 2b               \n"
                           "1:                                      \n" : [col] "+r"(col_base),
                           [sum] "=Te"(sum_tmp), [row0] "+r"(lhs_vec), [out0] "=Te"(acc_n0) : [cnt] "r"(rhs_cols) : "q0", "q1", "memory", "r14");
#endif
            sum_tmp *= lhs_offset;
            sum_tmp += acc_n0;
            if (bias)
    ...
    int8_t *new_dst = dst + rows_processed;
