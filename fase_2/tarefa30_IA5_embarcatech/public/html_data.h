...
const char *html_data =
    ...
    ".rain-badge{display:inline-block;padding:.25rem .6rem;border-radius:9999px;font-weight:600;font-size:.85rem}"
    ...
    "</header>"
    "<section class='mb-8'>"
    "<div class='data-card border-l-4 border-indigo-500 relative overflow-hidden'>"
    "<div class='absolute top-0 right-0 p-4 opacity-10'><i class='fas fa-binoculars text-8xl'></i></div>"
    "<div class='flex flex-col md:flex-row items-center justify-between z-10 relative'>"
    "<div class='text-center md:text-left mb-4 md:mb-0'>"
    "<h2 class='text-xl font-bold text-gray-300 mb-1'>Previsão Estimada (Próx. Hora)</h2>"
    "<p class='text-sm text-gray-500'>Baseada na tendência de pressão e umidade local</p>"
    "</div>"
    "<div class='flex items-center space-x-6'>"
    "<div class='text-center'><i id='forecast-icon' class='fas fa-question weather-icon-anim text-5xl text-gray-400'></i></div>"
    "<div class='text-left'>"
    "<p id='forecast-title' class='text-3xl font-bold text-white'>Aguardando...</p>"
    "<p id='forecast-desc' class='text-indigo-400 font-medium'>--</p>"
    "<p id='rain-prob' class='text-sm text-gray-300 mt-1'>Probabilidade: 0,0%</p>"
    "<p id='rain-alert' class='rain-badge mt-2 bg-gray-700 text-gray-200'>Limiar 0.5: --</p>"
    "</div>"
    "</div>"
    "</div>"
    "</div>"
    ...
    "chart.render();"
    "function updateForecast(p,h,rainAlert,rainProb){const i=document.getElementById('forecast-icon'),t=document.getElementById('forecast-title'),d=document.getElementById('forecast-desc');let f={text:'--',icon:'fa-question',color:'text-gray-400',desc:'--'};if(rainAlert===true){f={text:'Probabilidade chuva',icon:'fa-cloud-showers-heavy',color:'text-blue-400',desc:''}}else if(rainAlert===false){f={text:'Sol',icon:'fa-sun',color:'text-yellow-300',desc:''}}t.textContent=f.text;d.textContent=f.desc;i.className='fas '+f.icon+' weather-icon-anim text-5xl '+f.color}"
    ...
    "async function updateData(){try{const r=await fetch('/api/weather'),d=await r.json(),newTemp=d.temperature,newHumidity=d.humidity,newPressure=d.pressure,newAltitude=d.altitude,rainProb=d.rainProbability,rainAlert=d.rainAlert;if(!state.userEditing){if(d.maxTemperature!==undefined){state.maxLimit=d.maxTemperature;document.getElementById('max-temp').value=d.maxTemperature}if(d.minTemperature!==undefined){state.minLimit=d.minTemperature;document.getElementById('min-temp').value=d.minTemperature}if(d.tempOffset!==undefined){state.offset=d.tempOffset;document.getElementById('temp-offset').value=d.tempOffset}}const tempWithOffset=newTemp+state.offset;document.getElementById('temp-value').textContent=tempWithOffset.toFixed(1)+' °C';document.getElementById('humidity-value').textContent=Math.round(newHumidity)+' %';document.getElementById('pressure-value').textContent=Math.round(newPressure)+' hPa';document.getElementById('altitude-value').textContent=Math.round(newAltitude)+' m';updateForecast(newPressure,newHumidity,rainAlert,rainProb);const rainProbEl=document.getElementById('rain-prob');if(rainProb!==undefined&&rainProb!==null){const formatted=Number(rainProb*100).toFixed(1).replace('.',',');rainProbEl.textContent='Probabilidade: '+formatted+'%'}else{rainProbEl.textContent='Probabilidade: 0,0%'}const rainAlertEl=document.getElementById('rain-alert');if(rainAlert===true){rainAlertEl.textContent='Limiar 0.5: chuva provavel';rainAlertEl.className='rain-badge mt-2 bg-red-600/20 text-red-300'}else if(rainAlert===false){rainAlertEl.textContent='Limiar 0.5: sem chuva';rainAlertEl.className='rain-badge mt-2 bg-emerald-600/20 text-emerald-300'}else{rainAlertEl.textContent='Limiar 0.5: --';rainAlertEl.className='rain-badge mt-2 bg-gray-700 text-gray-200'}const now=new Date();document.getElementById('last-update').innerHTML='<i class=\"fas fa-clock text-blue-400\"></i> Última atualização: '+now.toLocaleTimeString('pt-BR');chartData.temp.push(tempWithOffset);chartData.humidity.push(newHumidity);chartData.pressure.push(newPressure);chartData.categories.push(now.toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit',second:'2-digit'}));if(chartData.temp.length>20){chartData.temp.shift();chartData.humidity.shift();chartData.pressure.shift();chartData.categories.shift()}updateChartSeries()}catch(e){console.error('Erro:',e)}}"
